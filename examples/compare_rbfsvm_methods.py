...
Xs = np.random.multivariate_normal([np.random.random() \
        * meandistance, np.random.random() * meandistance], cov, (N // gaussians,))
for i in range(gaussians - 1):
    Xs = np.vstack((Xs, np.random.multivariate_normal([np.random.random() \
        * meandistance, np.random.random() * meandistance], cov, (N // gaussians,))))

# cut data into XOR
ytrue = ((Xs[:, 0] < np.mean(Xs[:, 0])) * (Xs[:, 1] < np.mean(Xs[:, 1])) +
         (Xs[:, 0] > np.mean(Xs[:, 0])) * (Xs[:, 1] > np.mean(Xs[:, 1]))) * 1
...
sidx = random.sample(
    list(
        np.where(
            ytrue == 0)[0]),
    supevised_data_points // 2) + random.sample(
    list(
        np.where(
            ytrue == 1)[0]),
...
print(lbl)
model = sklearn.svm.SVC(kernel=kernel, probability=True, gamma='auto')
