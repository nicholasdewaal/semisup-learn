...
class QN_S3VM_Dense:
    ...
    def __getFitness_Prime(self, c):
        ...
        preds_unlabeled_squared_exp_f = multiply(
            preds_unlabeled, preds_unlabeled)
        preds_unlabeled_squared_exp_f = np.exp(
            -self.__s * preds_unlabeled_squared_exp_f)
        preds_unlabeled_squared_exp_f = multiply(
            preds_unlabeled_squared_exp_f, preds_unlabeled)
    ...
    def __recomputeModel(self, indi):
        self.__c = array(indi[0])[newaxis].T

    def __getTrainingPredictions(self, X, real_valued=False):
        preds = self.__KNR @ self.__c[0:self.__dim -
                                      1, :] + self.__c[self.__dim - 1, :]
        if real_valued == True:
            return preds.flatten(order='C').tolist()
        else:
            return np.sign(np.sign(preds) + 0.1).flatten(order='C').tolist()
...
class QN_S3VM_Sparse:
    ...
    def __init__(self, X_l, L_l, X_u, random_generator, ** kw):
        ...
        # in case the highest feature indices appear only in one of the two
        # data matrices
        if X_l.shape[1] > X_u.shape[1]:
            X_u = sparse.hstack([X_u, sparse.coo_matrix(
                X_u.shape[0], X_l.shape[1] - X_u.shape[1])])
        elif X_l.shape[1] < X_u.shape[1]:
            X_l = sparse.hstack([X_l, sparse.coo_matrix(
                X_l.shape[0], X_u.shape[1] - X_u.shape[1])])
        # We vertically stack the data matrices into one big matrix
        X = sparse.vstack([X_l, X_u])
        self.__size_l, self.__size_u, self.__size_n = X_l.shape[
            0], X_u.shape[0], X_l.shape[0] + X_u.shape[0]
        x = arr.array('i')
        for l in L_l:
            x.append(int(l))
        self.__YL = array(x, dtype=float64)
    ...
    def getPredictions(self, X, real_valued=False):
        ...
        # Again, possibility of dimension mismatch due to use of sparse
        # matrices
    ...
    def __setParameters(self, ** kw):
        ...
        self.__lamU = float(self.parameters['lamU'])
        assert self.__lamU > 0
        self.__lam_Uvec = [ self.__lamU * i for i in [ 0, 0.000001, 0.0001, 0.01, 0.1, 0.5, 1]]
        self.__minimum_labeled_patterns_for_estimate_r = float(
            self.parameters['minimum_labeled_patterns_for_estimate_r'])
        # If reliable estimate is available or can be estimated, use it, otherwise
        # assume classes to be balanced (i.e., estimate_r=0.0)
        if self.parameters['estimate_r'] is not None:
            ...
            self.__estimate_r = (
                1.0 / self.__YL.shape[0]) * np.sum(self.__YL[0:])
        else:
            self.__estimate_r = 0.0
    ...
    def __getFitness(self, c):
        ...
        preds_labeled = self.__surrogate_gamma * \
            (1.0 - multiply(self.__YL, (self.X_l@
                                        XTc - self.__mean_u@XTc) + b[0, 0]))
    ...
    def __getFitness_Prime(self, c):
        ...
        preds_unlabeled_squared_exp_f_sum = np.sum(
            preds_unlabeled_squared_exp_f)
        tmp_unlabeled = self.X_u_T * preds_unlabeled_squared_exp_f - \
            self.__mean_u.T * preds_unlabeled_squared_exp_f_sum
        term2 = ((-2.0 * self.__s * self.__lamU) / float(self.__size_u)) * \
...
class DictLinearKernel():
    ...
    def computeKernelMatrix(self, data1, data2, symmetric=False):
        ...
        self.__km = None
        try:
            km = zeros((self._dim1, self._dim2), dtype=float64)
            if self._symmetric:
                for i in range(self._dim1):
                    ...
                    for j in range(i, self._dim2):
                        val = self.getKernelValue(
                            self._data1[i], self._data2[j])
                        km[i, j] = val
                        km[j, i] = val
                return km
            else:
    ...
    def computeKernelMatrix(self, data1, data2, symmetric=False):
        ...
        try:
            if self._symmetric:
                ...
                trnorms = np.diag(linearkm)[newaxis].T
                trace_matrix = trnorms @ ones((1, self._dim1), dtype=float64)
                ...
                linkm = self._data1 @ self._data2.T
                trnorms1 = []
                for i in range(m):
                    trnorms1.append((self._data1[i] * self._data1[i].T)[0])
                trnorms1 = array(trnorms1)[newaxis].T
                trnorms2 = []
                for i in range(n):
                    trnorms2.append((self._data2[i] * self._data2[i].T)[0])
                trnorms2 = array(trnorms2)[newaxis].T
                self.__km = trnorms1 @ ones((n, 1), dtype=float64).T
                self.__km = self.__km + \
                    ones((m, 1), dtype=float64) @ trnorms2.T
